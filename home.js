...
const clothCardWrapper= $("#cloth-card-wrapper");
const accCardWrapper= $("#acc-card-wrapper");

//create cloth cards function
const clothCard = (data) =>{
     const clothGrid= $("<div>");
     clothGrid.addClass("cloth-card");
     const hyperlink= $("<a>");
     hyperlink.attr('href','./product.html?pID='+data.id);
     clothGrid.append(hyperlink);
     const imageCloth= $('<img>');
     imageCloth.attr('src',data.preview);
     const cardDes = $('<div>');
     cardDes.addClass('card-des');
     const clothHead= $('<h2>');
     clothHead.text(data.name);
     const clothSub= $('<h4>');
     clothSub.text(data.brand);
     const para = $('<p>');
     para.text(data.price);
     cardDes.append(clothHead);
     cardDes.append(clothSub);
     cardDes.append(para);
     hyperlink.append(imageCloth);
     hyperlink.append(cardDes);

     return clothGrid;
};

$.get('https://5ee248c68b27f30016094891.mockapi.io/homepagedata',function(response){
     for(let i=0; i<5; i++){
         const cardWrapper= clothCard(response[i]);
         clothCardWrapper.append(cardWrapper);
     } 
     for(let j=5; j<response.length; j++){
         const accWrapper= clothCard(response[j]);
         accCardWrapper.append(accWrapper);
     }  
    });  
